...
Los regresores cualitativas se representan en R con *factores*. En este caso hay dos variables cualitativas, *mom_hs* y *mom_work*. Como no son factores, se van a convertir a factor:
...
$$
kid\_score_i = b_0, i \in 1,2,\cdots,n_0
$$

donde $n_0$ es el numero de madres sin secundaria. Si sumamos en ambos lados del modelo se tiene:

$$
\sum_{i=1}^{n_0} kid\_score_i = \sum_{i=1}^{n_0} b_0 = n_0 b_0 \Rightarrow b_0 = \frac{\sum_{i=1}^{n_0} kid\_score_i}{n_0}
$$
...
$$
kid\_score_i = b_0 + b_1, i \in 1,2,\cdots,n_1
$$

donde $n_1$ es el numero de madres con secundaria. Si sumamos en ambos lados del modelo se tiene:

$$
\sum_{i=1}^{n_1} kid\_score_i = \sum_{i=1}^{n_1} (b_0 + b_1) = n_1 (b_0 + b_1) \Rightarrow b_0 + b_1 = \frac{\sum_{i=1}^{n_1} kid\_score_i}{n_1}
$$
...
$$
kid\_score = b_1 secundaria\_si + \beta_2 secundaria\_no + e
$$
...
```{r}
m = lm(kid_score ~ mom_iq + secundaria_si + mom_iq:secundaria_si, data = d)
summary(m)
```
...
abline(a = m$coefficients["(Intercept)"], b = m$coefficients["mom_iq"], col = "black")
abline(a = m$coefficients["(Intercept)"] + m$coefficients["secundaria_si"],
       b = m$coefficients["mom_iq"] + m$coefficients["mom_iq:secundaria_si"], col = "red")
```
...
Con factores, la interacción entre variables se incluye también con los dos puntos:
...
Otra alternativa es utilizar el signo de multiplicación:
